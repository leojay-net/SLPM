...
export class RealAtomiqSwapClient implements AtomiqSwapClient {
    ...
    constructor(network: 'MAINNET' | 'TESTNET' = 'TESTNET', starknetRpc?: string) {
        ...
        this.initializeAtomiqFactory().catch(error => {
            console.warn('⚠️ Atomiq initialization failed:', error);
            this.setupTestMode();
        });
    }
    ...
    private async initializeAtomiqFactory(): Promise<void> {
        try {
            ...
            console.log('✅ Using default storage (IndexedDB for browser, memory for server)');

            // Create swapper using factory with proper configuration
            this.swapper = this.factory.newSwapper(swapperConfig);

            console.log('✅ Atomiq Swapper created via factory');

            // Initialize the swapper
            await this.swapper.init();
            console.log('✅ Atomiq Swapper initialized');

            this.initialized = true;

        } catch (error) {
            console.error('❌ Failed to initialize real Atomiq SDK:', error instanceof Error ? error.message : String(error));
    ...
    async getQuote(
        ...
        try {
            ...
            console.log(`🔄 Creating Atomiq swap quote: ${from} -> ${to}, amount: ${amount}, exactIn: ${exactIn}`);
            ...
            if (from === 'BTC_LN') {
                ...
                console.log(`⚡ Lightning -> STRK swap created, invoice will be: ${swap.getAddress()}`);
            } else if (to === 'BTC_LN') {
                ...
                console.log(`⚡ STRK -> Lightning swap created, invoice required in execute()`);
            } else {
                ...
                console.log(`🔄 On-chain swap created: ${from} -> ${to}`);
            }
            ...
            console.error('❌ Atomiq SDK swap creation failed:', error);
    ...
    async execute(quoteId: string, walletSigner?: any, lightningInvoice?: string): Promise<AtomiqSwapExecution> {
        ...
        try {
            ...
            console.log(`⚡ Executing Atomiq swap ${quoteId}`);
            ...
            if (lightningInvoice && this.isLightningDestination(swap)) {
                ...
                console.log(`⚡ Committing swap with Lightning invoice destination`);
            ...
            if (result) {
                ...
                console.log(`✅ Swap ${quoteId} completed successfully`);
                if (bitcoinTxId) {
                ...
                console.warn(`⚠️ Swap ${quoteId} failed, attempting refund`);
                try {
            ...
            console.error('❌ Atomiq execution failed:', error);
            return {
                id: quoteId,
                status: 'FAILED',
